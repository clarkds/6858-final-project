...
def write_secrets():
	...
	try:
		...
		enc_pickle=crypt.sym_enc(client.passw, pickled)[1]
...
def load_secrets():
	try:
		...
		decrypted_pickle=crypt.sym_dec(client.passw, secre_file.read())
...
def api_login_helper(user, passw, secretsFile):	# LEO
	...
	if secretsFile is not None:
		try:
			shutil.copy(src, "data/"+client.user+"/secrets")
		except:
			traceback.print_exc()
			return False
...
def api_fopen(path, mode):
	...
	if enc_path not in client.keys:
		update_keys()
		if enc_path not in client.keys:
			if mode == "r":
				return False
