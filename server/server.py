	...
	def handle(self):
		...
		while logged_in:
			...
			if op == 'logoutUser':
				...
				path = ('users' + msg_obj['PATH'])
				parent_dir = get_parent_directory(path)
				meta_file = get_metafile_path(parent_dir)
				...
				if os.path.exists(path):
					...
					add_write_key(path, msg_obj['SECRET'])
					print meta_file
					add_write_key(meta_file, msg_obj['SECRET'])
					os.mkdir(path)
				else:
					response['ERROR'] = "Incorrect secret for " + parent_dir
					response['STATUS'] = 1
				...
				if os.path.exists(path) and os.path.isdir:
					...
					response["FILES"] = [f for f in files if (not f.startswith('.log_') and not f.startswith('.meta_'))]
		...
		try:
			...
			if op == 'createUser':
				...
				if os.path.exists('users/' + username) or user_exists(username):
					...
					add_write_key("users/.meta_" + username, msg_obj["SECRET"])
